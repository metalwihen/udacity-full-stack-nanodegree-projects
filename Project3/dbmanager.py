...
QUERY_DAYS_WITH_HIGH_ERRORS = """
		select
			...
			to_char((((errors_by_day.count::decimal/requests_by_day.count::decimal))*100),'9.99') || '%' as percentage
		from
			(select date(time),count(*) from log group by date(time)) as requests_by_day,
			(select date(time),count(*) from log where status != '200 OK' group by date(time)) as errors_by_day
		where
